...
function progress(message, percentage, options = {}) {
    ...
    const bar = `${`${config.symbols.progress[0]}`.repeat(completedLength)}${`${config.symbols.progress[1]}`.repeat(barWidth - completedLength)}`;
    const progressMessage = useBrackets
        ? `${config.colors.progress}[ ${bar} ] ${percentage}% ${colors.reset} ${message}`
        : `${config.colors.progress}${bar}  ${percentage}% ${colors.reset} ${message}`;
