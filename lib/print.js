...
function warn(message, options = { useBrackets: true, keepColoring: false, prefixCharacter: '' }) {
    console.log(formatLog('warning', message, options));
}

function error(message, options = { useBrackets: true, keepColoring: false, prefixCharacter: '' }) {
    console.log(formatLog('error', message, options));
}

function success(message, options = { useBrackets: true, keepColoring: false, prefixCharacter: '' }) {
    console.log(formatLog('success', message, options));
}

function info(message, options = { useBrackets: true, keepColoring: false, prefixCharacter: '' }) {
    console.log(formatLog('info', message, options));
}

function debug(message, options = { useBrackets: true, keepColoring: false, prefixCharacter: '' }) {
    console.log(formatLog('debug', message, options));
}

let loadingSessions = [];

function loading(message = '', options = { useBrackets: true, keepColoring: false, prefixCharacter: '', intervalSpeed: 500, onFinish: '' }) {
    const { useBrackets, keepColoring, prefixCharacter, intervalSpeed, onFinish } = options;
...
function progress(message, percentage, options = { useBrackets: true, barWidth: 20, prefixCharacter: '', onFinish: null }) {
    const { useBrackets, barWidth, prefixCharacter, onFinish } = options;
    ...
    const progressMessage = useBrackets
        ? `${progressBracketColor}[ ${bar} ${progressBracketColor}]  ${colors.bold}${percentage}% ${colors.reset} ${message}`
